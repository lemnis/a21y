...
function get(current: Roletype, traversedElements: Element[]): String | Element[] {
	
	if (
		(current.element as any).labels && (current.element as any).labels.length > 0 &&
		traversedElements.indexOf(current.element) === -1
	) {
		return Array.from(((current.element as any).labels as Element[]))
		.filter(element => traversedElements.indexOf(element) === -1);
	}
